<template>
  <div class="content_container">
    ...
    <dialogue :show-dialog="showDialog" :msg="msg" @onOk="showDialog = false"></dialogue>
  </div>
</template>
    ...
    methods: {
      ...
      modifyPassword(){
        this.$refs.form.validate(valid => {
          if(valid){
              let flag = passwordValid(store.getters.userInfo.jobNumber,this.form.newPassword);
              if (flag != 'correct'){
                this.msg = flag;
                this.showDialog = true;
              }else{
                let $qs = require('querystring') //nodejs自带querystring模块
                this.form.userId = store.getters.userInfo.id;

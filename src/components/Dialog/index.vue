<template>
  <div class="dialog" transition="fade">
    ...
    <div style="width: 25%;min-width:300px;height:100%;position: fixed;background: #fff;top:0;right: 0;z-index: 7;box-shadow: 1px 2px 10px 0 rgba(23, 21, 21,0.5);" >
      <div style="height: 51px;line-height: 51px;margin-left: 20px;font-size: 18px;font-family: 微软雅黑">
        {{ modal.title }}
      </div>
      <hr>
      <div style="height: calc(100% - 138px) ;overflow: auto">
        <slot name="name1" class="dialog_content" ></slot>
      </div>
      <div style="position: absolute;width:100%;height:60px;background:#eee;bottom: 0;text-align: center;padding-top: 10px">
        <el-button type="primary" :loading="loading" size="mini" @click="submit">{{modal.confirmButtonText  }}</el-button>
        <el-button size="mini"  @click="cancel">{{ modal.cancelButtonText }}</el-button>
      </div>
